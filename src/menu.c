...
int menu_ossetup_termios() {
  ...
  new_termios.c_iflag &= ~(ICRNL | INPCK | ISTRIP);
...
void menu_appendconsole(MenuConsole *mc, char* str) {
  ...
  if ((*mc).data == NULL) {
    ...
    for (; cap < (length + 1);)
      cap *= 2;
    (*mc).cap = cap;
    (*mc).data = ptr_create(sizeof(MenuList) * (*mc).cap + 1);

    strncpy((*mc).data, str, (*mc).cap);
    (*mc).len = length;
    return;
  }
...
int menu_appendlistn(MenuList *m, char *str) {
  if (m == NULL) {
    // TODO: error
    return -1;
  }
...
int menu_appendlist(MenuList *m, char *str) {
  return (menu_appendlistn(m, str) == -1) ? 1 : 0;
}
...
void menu_keys(MenuState *state, char *s, char *d, char *u, char *b) {
  (*state).keys[0] = s;
  (*state).keys[1] = d;
  (*state).keys[2] = u;
  (*state).keys[3] = b;
